...
| **Secreto JWT como variable de entorno de Lambda** (no Secrets Manager) | Secrets Manager cobra pasados los primeros 30 días; SSM `SecureString` no se puede crear vía CloudFormation. Se genera un valor aleatorio en tiempo de síntesis de CDK y se inyecta como env var (cifrada en reposo con la KMS key gestionada de Lambda, sin costo). Con más tiempo/presupuesto: Secrets Manager con rotación. |
